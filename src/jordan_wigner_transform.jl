        ...
        for j in 1:norb
            if i != j
                for k in 1:norb
                    for l in 1:norb
                        if k != l
                            oper = jordan_wigner(i,norb) * jordan_wigner(j,norb) * adjoint(jordan_wigner(k,norb)) * adjoint(jordan_wigner(l,norb))
                            oper = oper * h2[i,j,k,l]
                            op += clip!(oper)
                        end
                    end
                end
...
function transform_molecular_Hamiltonian(data_dir="H2_0.75")
    ...
    qubit_ham = clip!(one_body_transform(h1) + two_body_transform(h2), thresh=1e-15)
